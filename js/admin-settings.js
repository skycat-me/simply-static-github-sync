...
jQuery(document).ready(function($) {
  ...
  $("#sistContainer input[type=radio][name=destination_url_type]").change(
    function() {
  ...
  $(
    "#sistContainer input[type=radio][name=destination_url_type]:checked"
  ).change();
  ...
  function destination_url_type_change($this) {
    ...
    if ($radio.val() == "absolute") {
      $("#destinationHost").prop("disabled", false);
      $("#destinationScheme").prop("disabled", false);
    } else {
      $("#destinationHost")
        .val("")
        .prop("disabled", true);
      $("#destinationScheme").prop("disabled", true);
    }
  ...
  $("#AddUrlToExclude").click(function() {
    ...
    var $clone_row = $("#excludableUrlRowTemplate")
      .clone()
      .removeAttr("id");
    ...
    $clone_row.html(
      $clone_row.html().replace(regex, "excludable[" + timestamp + "]")
    );
  ...
  $("#AddGithubFileToExclude").click(function() {
    var $last_row = $(".excludable-file-row").last();
    var $clone_row = $("#excludableFileRowTemplate")
      .clone()
      .removeAttr("id");

    var timestamp = new Date().getTime();
    var regex = /github_excludable\[0\]/g;

    $clone_row.html(
      $clone_row.html().replace(regex, "github_excludable[" + timestamp + "]")
    );
    $clone_row.insertAfter($last_row);
  });

  $("#excludableFileRows").on(
    "click",
    ".remove-excludable-file-row",
    function() {
      var $row = $(this).closest(".excludable-file-row");
      $row.remove();
    }
  );
});
